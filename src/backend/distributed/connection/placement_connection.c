							 ...
							 const char *userName)
{
	...
	ShardPlacement *lastPlacement = NULL;
	...
	foreach(placementAccessCell, placementAccessList)
	{
		...
		{
			/* we already found a connection that performed writes and must use it */
		...
		{
			/* use the connection assigned to the current placement */
			connection = placementConnection->connection;
	...
	if (connection == NULL)
	{
		 ...
		 */
		connection = StartNodeConnection(flags, lastPlacement->nodeName,
										 lastPlacement->nodePort);
	}
			...
			placementEntryCell, placementEntryList)
	{
		...
		else
		{
			...
			if (accessType != PLACEMENT_ACCESS_SELECT)
			{
				 ...
				 * it. Update the connection reference to point to the connection
				 * used for writing. We don't need to remember the existing connection
				 * since we won't be able to reuse it for accessing the placement.
				 * However, we do register that it exists in hasSecondaryConnections.
				 */
				placementConnection->connection = connection;
				placementConnection->userName = MemoryContextStrdup(TopTransactionContext,
																	userName);

				AssertArg(!placementConnection->hadDDL);
				AssertArg(!placementConnection->hadDML);
			}
